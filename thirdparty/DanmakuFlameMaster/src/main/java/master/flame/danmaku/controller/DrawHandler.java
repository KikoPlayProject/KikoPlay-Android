...
public class DrawHandler extends Handler {
    ...
    private final long syncTimer(long startMS) {
        ...
        if (mNonBlockModeEnable) {
            ...
            // 保留平滑推进 timer.add(d)，避免帧间隔抖动直接传导导致弹幕顿挫。
            // 但把"墙钟多走、timer 没推"的债 (gapTime - d) 转嫁给 mTimeBase，并清零
            // mRemainingTime，使下一帧 time 紧跟 timer、且与 nothingRendered/waiting
            // 分支 (update(time)) 一致，消除弹幕移出屏幕时 A/B 切换的时钟跳变。
            // 详见 GitHub issue #426。
            mTimeBase += (gapTime - d);
            mRemainingTime = 0;
            timer.add(d);
