...
- 本地视频扫描与播放
- 视频内嵌字幕与 PC Web 字幕显示，并支持在横屏播放设置内切换字幕或关闭字幕；远程字幕会先从 `/sub/{format}/{mediaID}` 拉取原文并做本地兼容规范化后再交给 Media3；应用设置中的“播放”分组提供小窗播放、字幕样式与字幕大小预设，小窗模式下可继续显示字幕
- 弹幕加载、增量刷新、来源信息查看与同步调整、按来源显示或隐藏弹幕、存在弹幕池时发送与基础设置
...
| 弹幕 | DanmakuFlameMaster (本地 fork) | 0.9.25 |
| 图片加载 | Coil 3 | 3.1.0 |
...
- `settings.gradle.kts` 中保留 JitPack 仓库配置；`DanmakuFlameMaster` 已改为本地 fork 后该仓库不再有实际依赖，仅作历史保留
...
| `thirdparty/` | 第三方库的本地 fork，当前包含 `DanmakuFlameMaster`（修复 `DrawHandler.syncTimer` 弹幕时钟跳变，详见对应章节） |
...
```text
thirdparty/
└── DanmakuFlameMaster/      # DFM v0.9.25 本地 fork
    ├── build.gradle.kts      # 适配当前 AGP 的构建脚本（compileSdk 36 / Java 11，无外部依赖）
    └── src/main/java/        # DFM flame module 完整源码（原样保留）
```

`DanmakuFlameMaster` 由原先的 JitPack maven 依赖改为本地源码 module，仅用于修改 `DrawHandler.syncTimer` 以修复弹幕时钟跳变（见「弹幕时钟跳变修复」章节）。源码原样保留未做删减，仅改 `syncTimer` 一处；原始 native（`ndkbitmap`）依赖已移除，因 `minSdk 24` 下其加载条件 `11 <= SDK_INT < 23` 永不满足，`NativeBitmapFactory` 始终走 Java fallback，功能无损失。
...
- `data/repository`: 面向 ViewModel 的仓库层封装，包含历史续播目标解析、缓存 / PC 播放上下文衔接、播放列表与 PC 最近播放缓存、历史缩略图生成、观看历史串行写入以保留退出播放器时的最终帧缩略图、首页最近观看复用、启动统计上报，以及设置页 / 播放器页偏好的 DataStore 持久化逻辑；其中 `PlaylistRepository` 会在拉取 `/api/playlist` 时读取响应头 `X-Kiko` 并记录远端版本，供后续能力兼容判断使用；`HiddenDanmakuSourceStore` 以会话级内存单例按弹幕池隔离记录被隐藏的弹幕来源（sourceId 仅在弹幕池内有效，故不持久化，退出即清空，覆盖同一番剧跨集复用）

### UI 层

- `ui/home`: 首页与观看历史入口，首页最近观看使用纵向缩略图列表展示，包含 PC 最近播放与 App 本地历史合并、最近观看去重、缓存标记、进度条与恢复播放；PC 最近播放只作为首页合并来源，不写入 Room，后续 App 播放记录会按本地更新时间正常排到前面，且同一条目合并展示时优先使用 App 本地历史保存的进度、时长与缩略图；当最近观看条目已缓存时，会优先使用本地缓存进入播放器，同时尽量保留对应 PC 同目录剧集上下文
...
- `ui/player`: 播放器、同目录剧集列表、进入播放页后剧集列表自动定位当前播放项、剧集长按多选缓存、续播进度同步、自动下一集、纵向布局下支持“剧集 / 弹幕”分页与左右滑动切换；当剧集列表为空时隐藏剧集页并直接显示弹幕页；在横向宽屏设备上，非全屏状态会改用左侧播放器 / 右侧内容栏的双栏布局，以便同时展示剧集与弹幕信息；弹幕来源卡片展示与编辑、按来源显示或隐藏弹幕（会话级按弹幕池记忆，隐藏时仅过滤该来源、统计信息仍可见）、弹幕增量刷新、延迟与时间轴同步调整、无弹幕池时隐藏发送弹幕入口，以及弹幕与截图片段交互；已完成缓存的剧集会在列表中显示“已缓存”标记，当当前或目标剧集存在本地可播放缓存时，剧集切换与自动下一集会优先使用缓存播放，否则继续走 PC 串流。支持 Media3 文本轨道显示视频内嵌字幕与远程 Web 字幕，远程字幕会先从 `/sub/{format}/{mediaID}` 拉取原文并做本地兼容规范化后再交给 Media3，默认优先选择第一个内嵌字幕、无内嵌字幕时回退到 `/api/subtitle` 返回字幕，横屏“设置 > 播放设置”可在“无 / 内嵌字幕 / Web 字幕”之间切换；播放器会根据应用设置中的字幕样式与字幕大小预设应用 `SubtitleView` 显示风格，当前提供跟随系统、黑底白字、半透明黑底白字、白字描边、白字阴影、黄字描边与黄字阴影等预置样式，小窗模式下会保留字幕显示但继续关闭弹幕层。支持普通播放页黑色系统栏、全屏/横屏隐藏系统栏、横屏自动下一集时保持横屏与隐藏系统栏、传感器横屏 180 度翻转、顶部/底部渐变控制层、横向手势精细 seek、横屏顶部标题区域手势排除、左右三分区上下滑分别调节亮度与音量、中间三分区上下滑不触发亮度/音量、下半区域长按 1 秒临时 2 倍速并带轻震动反馈与居中偏下的轻量 `2.0x` 提示，横屏时会在顶部中央显示当前时间，并会在竖屏下为竖版视频提供独立的竖版全屏入口，弹幕显示开关、显示区域、不透明度、字体大小、弹幕速度、类型屏蔽与播放速度会持久化恢复，同时“同步播放进度”设置会实际控制是否向 PC 端回传进度；播放器在退出/播完/切集等终态同步时会把当前帧 PNG 缩略图一并回传到 `/api/updateTime.preview`，但只有在远端 Web API 响应头携带 `X-Kiko` 且版本号不低于 `200100` 时才会附带该字段，旧版本或未声明能力的服务端仍只同步时间本身。播放器在实际播放或缓冲待播时保持屏幕常亮，自动跳转下一集期间持续保持常亮不中断，暂停后恢复系统息屏策略；弹幕渲染保留 DanmakuFlameMaster 绘制缓存，seek 时立即把弹幕时钟冻结到目标位置并在 ExoPlayer 真正恢复播放（`onIsPlayingChanged`）时才 resume，避免 buffering 期间弹幕时钟按墙钟空跑累积漂移，对 `/api/danmu/full/` 返回的来源裁剪、时间轴与延迟按 PC 端一致顺序进行本地换算（先裁剪并减去裁剪起点，再累计应用时间轴偏移，最后加整体延迟），使用自定义滑杆组件并按原视频比例渲染画面，同时会在退出播放器或自动切到下一集前保存当前帧历史缩略图，并在低内存设备上启用更保守的缓冲、seek、抓图与播放错误提示兜底
- `ui/local`: 本地视频列表
...
└── java/com/kiko/kikoplay/
    ...
    ├── ui/player/subtitle/
    │   ├── RemoteSubtitleNormalizerTest.kt
...
- `RemoteSubtitleNormalizerTest.kt` 覆盖远程 SRT 字幕时间轴兼容规范化
- 其余两个测试文件仍是模板级示例
...
弹幕滚动时偶发"向后闪现"（弹幕滑到快要消失时突然回退一段）曾长期存在，根因有两层：

1. **seek 后弹幕时钟漂移**：原实现在 seek 后调用 `danmakuView.seekTo(target)` 但未立即暂停，ExoPlayer 进入 buffering 期间弹幕时钟仍按墙钟空跑，与媒体时钟拉开差距；且 play/pause 同步依赖 `delay(300)` 去抖，buffering 短于 300ms 时暂停被取消，漂移进一步累积。
2. **DFM `syncTimer` 双分支时钟跳变**（GitHub issue #426）：DFM 在有弹幕渲染时用 `timer.add(d)` 平滑节流推进、累积 `mRemainingTime` 偏差；无弹幕渲染（弹幕移出屏幕）时改用 `timer.update(time)` 直接对齐墙钟并清零 `mRemainingTime`。弹幕移出屏幕瞬间触发 A→B 分支切换，`mRemainingTime` 被清零导致时钟突跳，在屏最后一条弹幕回弹。弹幕稀疏时频繁切换故尤为明显。

对应修复分三层：

- **seek 后立即暂停弹幕时钟**（`VideoPlayerScreen.performSeek`）：`seekTo` 后紧跟 `pause()`，让 DFM 冻结在目标位置，直到 ExoPlayer 真正恢复播放（`onIsPlayingChanged`）才 resume。
- **play/pause 改由 `onIsPlayingChanged` 驱动**：去掉 `delay(300)` 去抖，pause 立即执行、resume 仅保留 100ms 轻去抖。`isPlaying` 来自 ExoPlayer，仅在 `STATE_READY && playWhenReady` 时为 true，本身是干净二态。
- **fork DFM 修复 `syncTimer`**：A 支保留原 `timer.add(d)` 平滑推进（保流畅），但把"墙钟多走、timer 没推"的债 `(gapTime - d)` 转嫁给 `mTimeBase` 并清零 `mRemainingTime`，使债不累积且与 B 支一致，消除分支切换跳变。该 fork 即 `thirdparty/DanmakuFlameMaster`。

## 已知待完善项

1. KService gRPC 集成尚未完成
2. 弹幕时钟已修复 `syncTimer` 跳变与 seek 漂移，但长期播放下与 ExoPlayer 的残余墙钟漂移仍未做周期性软校准兜底
3. DanmakuFlameMaster 0.9.25（本地 fork）的类型过滤能力有限
